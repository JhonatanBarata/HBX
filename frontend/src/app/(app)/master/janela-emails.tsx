...
export function JanelaEmails() {
  ...
  return (
    <React.Fragment>
      {loadError && <div style={{ fontSize: "var(--fz-l3)", fontWeight: 600, color: "var(--hbx-danger)" }}>{loadError}</div>}
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {!templates && !loadError && <span style={{ fontSize: "var(--fz-l3)", color: "var(--text-muted)" }}>Carregando templates…</span>}
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 14, alignItems: "start" }}>
          <section className="panel">
            ...
            <div style={{ padding: "12px 16px 16px", display: "grid", gap: 12 }}>
              {msg && (
                <div style={{ fontSize: "var(--fz-m1)", fontWeight: 700, color: msg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-warning)" }}>{msg}</div>
              )}
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <section className="panel">
              ...
              <form onSubmit={enviarApresentacao} style={{ padding: "12px 16px 16px", display: "grid", gap: 10 }}>
                {envioMsg && (
                  <div style={{ fontSize: "var(--fz-m1)", fontWeight: 700, color: envioMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-warning)", lineHeight: 1.5 }}>{envioMsg}</div>
                )}
                {!estado?.attachment && (
            ...
            <section className="panel">
              ...
              <form onSubmit={enviarTeste} style={{ padding: "12px 16px 16px", display: "grid", gap: 10 }}>
                {testeMsg && (
                  <div style={{ fontSize: "var(--fz-m1)", fontWeight: 700, color: testeMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-warning)", lineHeight: 1.5 }}>{testeMsg}</div>
                )}
                <div style={{ display: "grid", gap: 6 }}>
            ...
            <section className="panel">
              ...
              <div style={{ padding: "12px 16px 16px", display: "grid", gap: 10, fontSize: "var(--fz-l3)" }}>
                ...
                {uploadMsg && (
                  <div style={{ fontSize: "var(--fz-m2)", fontWeight: 700, color: uploadMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-warning)" }}>{uploadMsg}</div>
                )}
                <div style={{ borderTop: "1px solid var(--border-hairline)", paddingTop: 10, display: "grid", gap: 8 }}>
                  <strong style={{ fontSize: "var(--fz-m1)" }}>Apresentação (.pptx)</strong>
                ...
                <div style={{ borderTop: "1px solid var(--border-hairline)", paddingTop: 10, display: "grid", gap: 8 }}>
                  <strong style={{ fontSize: "var(--fz-m1)" }}>Cartão de visitas (assinatura)</strong>
