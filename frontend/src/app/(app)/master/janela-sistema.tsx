...
export function JanelaSistema() {
  ...
  return (
    <React.Fragment>
      ...
      {sub === 0 && (
        ...
        <section className="panel">
          ...
          <div style={{ padding: "12px 16px 16px", display: "grid", gap: 14 }}>
            {credMsg && <div style={{ fontSize: "var(--fz-m1)", fontWeight: 700, lineHeight: 1.5, color: credMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-warning)" }}>{credMsg}</div>}
            ...
            <div style={{ display: "grid", gap: 8 }}>
              <strong style={{ fontSize: "var(--fz-l2)" }}>WhatsApp (Meta Cloud)</strong>
              {(waLib || []).length === 0 && <span style={{ fontSize: "var(--fz-m2)", color: "var(--text-muted)" }}>Sem credenciais — adicione a primeira.</span>}
              {(waLib || []).map((c, i) => (
            ...
            <div style={{ borderTop: "1px solid var(--border-hairline)", paddingTop: 12, display: "grid", gap: 8 }}>
              <strong style={{ fontSize: "var(--fz-l2)" }}>Mercado Pago</strong>
              {(mpLib || []).length === 0 && <span style={{ fontSize: "var(--fz-m2)", color: "var(--text-muted)" }}>Sem credenciais — adicione a primeira.</span>}
              {(mpLib || []).map((c, i) => (
      ...
      {sub === 1 && (
        <React.Fragment>
          {exclMsg && <div style={{ fontSize: "var(--fz-m1)", fontWeight: 700, color: exclMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-warning)" }}>{exclMsg}</div>}
          <section className="panel">
            <div className="panel-head">
