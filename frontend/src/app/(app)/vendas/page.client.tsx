        ...
        <div className="vnd-modehost" data-mode={modo} data-fx={EFFECTS_ON ? "on" : "off"}>
          ...
          <div className="vnd-stage">
            ...
            <section className="panel">
              {loadError && (
                <div style={{ padding: "12px 16px", fontSize: "var(--hbx-font-min)", fontWeight: 600, color: "var(--hbx-danger)" }}>
                  {loadError}
                </div>
              )}
      ...
      {novoOpen && (
        <div className="hbx-veil" onClick={e => { if (e.target === e.currentTarget) setNovoOpen(false); }}>
          <form className="hbx-modal" onSubmit={criarLead}
            ...
            {novoMsg && (
              <div style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: novoMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{novoMsg}</div>
            )}
            <div style={{ display: "grid", gap: 6 }}>
              <label style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--text-muted)" }}>Nome / Empresa</label>
            ...
            <div style={{ display: "grid", gap: 6 }}>
              <label style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--text-muted)" }}>Próximo passo</label>
              <input className="field-dark" maxLength={140} value={novoForm.nextAction}
                onChange={e => setNovoForm(f => ({ ...f, nextAction: e.target.value }))} />
            </div>
            <div style={{ display: "grid", gap: 6 }}>
              <label style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--text-muted)" }}>Nota</label>
          ...
          <div className="hbx-drawer" style={{ width: 340, height: "100vh", overflowY: "auto", padding: "18px 16px", display: "grid", gap: 14, alignContent: "start" }}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <label style={{ display: "grid", gap: 4, fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>
                  Início
              ...
              {podeConfigurarDisparo ? (
                <React.Fragment>
                  {comercialConfigMsg && (
                    <span style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: comercialConfigMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{comercialConfigMsg}</span>
                  )}
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              {podeConfigurarDisparo ? (
                <React.Fragment>
                  ...
                  {catalogoMsg && (
                    <span style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: catalogoMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{catalogoMsg}</span>
                  )}
      ...
      {agendaOpen && (
        <div className="hbx-veil to-right" onClick={e => { if (e.target === e.currentTarget) setAgendaOpen(false); }}>
          <div className="hbx-drawer" style={{ width: 340, height: "100vh", overflowY: "auto", padding: "18px 16px", display: "grid", gap: 14, alignContent: "start" }}>
            ...
            {syncMsg && (
              <div style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: syncMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{syncMsg}</div>
            )}
            ...
            {([["Atrasados", board?.blocks?.overdue || []], ["Hoje", board?.blocks?.today || []], ["Agendados", board?.blocks?.scheduled || []]] as [string, VendasLead[]][]).map(([label, cards]) => (
              <div key={label} style={{ display: "grid", gap: 8 }}>
                ...
                {cards.length === 0 && <span style={{ fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>Nenhum retorno.</span>}
                {cards
                  .slice()
                  ...
                  .map(card => (
                    <button key={card.id} onClick={() => { setSel(card); setAgendaOpen(false); }}
                      ...
                      <span style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                        <strong style={{ fontSize: "var(--hbx-font-min)" }}>{card.name || "—"}</strong>
            ...
            <div className="vnd-popup__body">
              ...
              <div className="vnd-popup__field">
                ...
                {retornoData && retornoHora && (
                  <span style={{ fontSize: "var(--hbx-font-min)", color: slotPreview?.conflito ? "var(--hbx-warning)" : "var(--text-muted)" }}>
                    {slotPreviewBusy
                      ? "Consultando agenda…"
                      : slotPreview
