  ...
  return (
    <div className="work" style={{ flex: 1 }}>
          <div className="rel-tabs glass-pill-track">
            ...
            {pdfError && <span style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--hbx-danger)" }}>{pdfError}</span>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 14 }}>
            <section className="panel">
              ...
              <div style={{ padding: "10px 18px 16px" }}>
                {segments.length === 0 && (
                  <p style={{ margin: "12px 0", fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>Sem dados no período — trabalhe alguns cards para o ranking aparecer.</p>
                )}
                {segments.length > 0 && (
            ...
            <section className="panel">
              ...
              <div style={{ padding: "16px 18px 18px" }}>
                {channels.length === 0 && (
                  <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>Sem dados de canal no período.</p>
                )}
                <div className="hbar">
                  {channels.map((c, i) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: 14 }}>
            <section className="panel">
              ...
              <div style={{ padding: "14px 18px 18px" }}>
                {funil.length === 0 && (
                  <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>Sem dados no período.</p>
                )}
                {funil.length > 0 && (
                  <React.Fragment>
                    ...
                    <div className="fleg">
                      {funil.map((f, i) => (
                        <div className="row" key={f.label}>
                          ...
                          <span style={{ marginLeft: "auto", color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)" }}>
                    ...
                    {report?.recommendation && (
                      <p style={{ margin: "12px 0 0", fontSize: "var(--hbx-font-min)", lineHeight: 1.5, color: "var(--text-muted)" }}>{report.recommendation}</p>
                    )}
                  </React.Fragment>
                )}
              </div>
            ...
            <section className="panel">
              <div className="panel-head">
                ...
                <div className="meta">
                  ...
                  {botAccessMsg && <span style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--hbx-brand-strong)" }}>{botAccessMsg}</span>}
                          ...
                          <tr onClick={() => toggleSeller(v.seller.id)} aria-expanded={open} title="Abrir vendedor">
                            <td>
                              <div style={{ display: "flex", gap: 9, alignItems: "center" }}>
                                <span aria-hidden style={{ display: "inline-block", width: 12, fontSize: "var(--hbx-font-min)", transform: open ? "rotate(90deg)" : "none", transition: "transform .15s" }}>▸</span>
    ...
    return (
      <section className="panel">
        ...
        <div style={{ padding: 18 }}>
          <span style={{ fontSize: "var(--hbx-font-min)", color: "var(--hbx-danger)", fontWeight: 700 }}>{err}</span>
        </div>
      </section>
    );
  }
          ...
          <div className="panel" style={{ padding: 0 }}>
            ...
            <div style={{ padding: "12px 16px 16px" }}>
              {(!fr || fr.stages.length === 0) && (
                <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>Sem cards no período.</p>
              )}
              {fr && fr.stages.length > 0 && (
            ...
            <div style={{ padding: "14px 16px 16px" }}>
              <div className="fleg">
                <div className="row">
                  ...
                  <span style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)", fontWeight: 700 }}>{bot?.touched ?? "—"}</span>
                </div>
                <div className="row">
                  <span className="swatch" style={{ background: "var(--hbx-warning)" }}></span>Devolvidas pro humano
                  <span style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)" }}>{bot ? `${bot.handedToHuman} (${botMax ? Math.round((bot.handedToHuman / botMax) * 100) : 0}%)` : "—"}</span>
                </div>
                <div className="row">
                  <span className="swatch" style={{ background: "var(--hbx-brand-strong)" }}></span>Viraram venda
                  <span style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--hbx-brand-strong)" }}>{bot ? `${bot.converted} (${botMax ? Math.round((bot.converted / botMax) * 100) : 0}%)` : "—"}</span>
                </div>
              </div>
            </div>
          </div>
