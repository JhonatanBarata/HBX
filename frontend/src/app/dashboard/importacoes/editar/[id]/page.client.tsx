...
export default function EditImportacaoClientPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push("/dashboard/importacoes/followup-global");
    } catch (e) {
  ...
  return (
    <DashboardScaffold title={`Editar pedido #${id || ""}`} description="Edite todos os dados do pedido" actions={<Link href="/dashboard/importacoes/cadastros" className="btn btn-secondary btn-sm">Cadastros</Link>}>
      ...
      {!loading ? (
        <form onSubmit={submit} className="panel p-4">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <Link href="/dashboard/importacoes/followup-global" className="btn btn-secondary">Voltar</Link>
          </div>
        </form>
      ) : null}
    </DashboardScaffold>
