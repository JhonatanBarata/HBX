    ...
    <div className="hbx-veil" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="hbx-modal" style={{ width: "min(420px, 100%)", display: "grid", gap: 14, padding: 24, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {prettyPhone(payload?.data?.phone) && <span style={{ fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)" }}>{prettyPhone(payload?.data?.phone)}</span>}
        </div>
        ...
        {payload?.message && (
          <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", color: "var(--text-muted)", lineHeight: 1.5 }}>{sanitizeJidsInText(payload.message)}</p>
        )}
        {error && (
          <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--hbx-danger)" }}>{error}</p>
        )}
        {bootstrapMsg && (
          <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", fontWeight: 700, color: bootstrapMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{bootstrapMsg}</p>
        )}
        ...
        {method === "qr" && status === "waiting_qr" && (
          <div style={{ display: "grid", justifyItems: "center", gap: 8, padding: 12, borderRadius: "var(--radius-md)", border: "1px dashed var(--border-strong)", background: "var(--hbx-surface-soft)" }}>
            {qr
              ...
              : <span style={{ fontSize: "var(--hbx-font-min)", color: "var(--text-muted)" }}>Gerando QR…</span>}
        ...
        {connected && (
          <>
            ...
            <div className="kv">
              <div className="row"><span className="k">Conectado em</span><span className="v" style={{ fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)" }}>{formatWhatsAppDateTime(payload?.data?.connectedAt)}</span></div>
              <div className="row"><span className="k">Atualizado em</span><span className="v" style={{ fontFamily: "var(--font-mono)", fontSize: "var(--hbx-font-min)" }}>{formatWhatsAppDateTime(payload?.data?.updatedAt)}</span></div>
            </div>
          </>
        )}
        ...
        <div style={{ display: "grid", gap: 8 }}>
          ...
          {connected && (
            <div style={{ marginTop: 4, paddingTop: 10, borderTop: "1px dashed var(--border-strong)", display: "grid", gap: 6 }}>
              ...
              {wipeMsg && (
                <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", color: wipeMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{wipeMsg}</p>
              )}
              {confirmWipe ? (
