...
// parando aos poucos, não piscar e voltar o outro". A rampa mora no hook
// useAnimationRate — o mini-radar do card de Vendas usa o MESMO freio.
export const RADAR_RATE: Record<RadarDiscState, number> = {
  funcionando: 1,
  pausado: 0.18,
  parado: 0,
};
...
export function RadarDisc({ mini = false, state = "funcionando" }: { mini?: boolean; state?: RadarDiscState } = {}) {
  ...
  // Inércia do disco: o ponteiro desacelera até congelar onde está (parado) ou
  // quase parar (pausado) e volta a acelerar do MESMO ponto no play.
  const wrapRef = useRef<HTMLDivElement>(null);
  useAnimationRate(wrapRef, RADAR_RATE[state] ?? 1);
