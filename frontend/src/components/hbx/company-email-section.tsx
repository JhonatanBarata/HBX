  ...
  return (
    <React.Fragment>
      <section className="panel">
        ...
        {loadError && <div style={{ padding: "12px 16px", fontSize: "var(--hbx-font-min)", fontWeight: 600, color: "var(--hbx-danger)" }}>{loadError}</div>}
        {!loadError && (
          <div style={{ padding: 18, display: "grid", gap: 14 }}>
            <p style={{ margin: 0, fontSize: "var(--hbx-font-min)", lineHeight: 1.55, color: "var(--text-muted)" }}>
            ...
            {estado?.hbxShared ? (
              <div style={{ padding: "10px 12px", borderRadius: "var(--radius-sm)", border: "1px solid color-mix(in srgb, var(--hbx-brand) 30%, transparent)", background: "var(--hbx-brand-soft)", fontSize: "var(--hbx-font-min)", lineHeight: 1.5 }}>
              ...
              <form onSubmit={salvarSmtp} style={{ display: "grid", gap: 12 }}>
                ...
                {!pronto && (estado?.sender?.missing?.length || 0) > 0 && (
                  <div style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: "var(--hbx-warning)" }}>
            ...
            {atual ? (
              <React.Fragment>
                ...
                {atual.requiredVariable && (
                  <span style={{ fontSize: "var(--hbx-font-min)", color: "var(--hbx-warning)", fontWeight: 700 }}>Este template precisa conter {atual.requiredVariable}.</span>
                )}
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  <strong style={{ fontSize: "var(--hbx-font-min)" }}>Envio de teste</strong>
                  {testeMsg && <span style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: testeMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{testeMsg}</span>}
        ...
        <section className="panel">
          ...
          <div style={{ padding: 18, display: "grid", gap: 12 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <label className="btn-ghost" style={{ minHeight: 30, fontSize: "var(--hbx-font-min)", cursor: "pointer", display: "inline-flex", alignItems: "center" }}>
              ...
              {estado?.attachment && (
                <button className="btn-ghost" style={{ minHeight: 30, fontSize: "var(--hbx-font-min)", color: "var(--hbx-danger)" }} disabled={uploadBusy !== null} onClick={() => removerAsset("attachment")}>Remover</button>
              )}
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {estado?.businessCard && (
                <button className="btn-ghost" style={{ minHeight: 30, fontSize: "var(--hbx-font-min)", color: "var(--hbx-danger)" }} disabled={uploadBusy !== null} onClick={() => removerAsset("business-card")}>Remover</button>
              )}
            </div>
          </div>
        </section>
      ...
      {ativo && (
        <section className="panel">
          ...
          <form onSubmit={enviarAvulso} style={{ padding: 18, display: "grid", gap: 10 }}>
            ...
            {envioMsg && <span style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700, color: envioMsg.startsWith("✓") ? "var(--hbx-brand-strong)" : "var(--hbx-danger)" }}>{envioMsg}</span>}
          ...
          <div className="hbx-modal" style={{ width: "min(560px, 100%)", display: "grid", gap: 12, padding: 24 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {(atual?.variableDefinitions || []).map(v => (
                <button key={v.key} className="btn-ghost" style={{ minHeight: 26, padding: "0 8px", fontSize: "var(--hbx-font-min)", fontFamily: "var(--font-mono)" }}
