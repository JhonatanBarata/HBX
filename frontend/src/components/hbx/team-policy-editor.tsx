...
const RISK_META: Record<AccessItem["riskLevel"], { label: string; cls: string }> = {
  low: { label: "Baixo", cls: "ok" },
  medium: { label: "Médio", cls: "teal" },
  high: { label: "Sensível", cls: "warn" },
  critical: { label: "Crítico", cls: "red" },
};
...
export function TeamPolicyEditor({ userId, sellers, isSelf }: {
  ...
  const defaultForItem = useCallback(
    (it: AccessItem) => (targetIsSeller ? Boolean(it.defaultForSeller) : Boolean(it.defaultForAdmin)),
    [targetIsSeller],
  );
  ...
  return (
    <div className="tp">
        ...
        <section className="tp-sec">
          ...
          <div className="tp-grid2">
            <div className="tp-field">
              <span className="lbl">Buscar permissão</span>
              <input className="field-dark" placeholder="Filtrar por nome, módulo ou descrição…"
                value={accessFilter} onChange={(e) => setAccessFilter(e.target.value)} />
            </div>
            {presets.length > 0 && (
              <div className="tp-field">
            ...
            {groups.map((g) => {
              const items = catalog
                ...
                .filter((it) => !q
                  ...
                  || String(it.requiresModule || "").toLowerCase().includes(q));
              if (!items.length) return null;
              const onCount = items.filter((it) => Boolean(accessDraft[it.key])).length;
              const collapsed = Boolean(collapsedGroups[g.key]) && !q;
              return (
                <div key={g.key} className="tp-group tp-group-wide">
                  <button type="button" className="tp-grouphead"
                    ...
                    <span className="meta">
                      <span className="tag">{onCount}/{items.length} ON</span>
                      <span className="car">{collapsed ? "▸" : "▾"}</span>
                    </span>
                  </button>
                  {!collapsed && items.map((it, idx) => {
                    const on = Boolean(accessDraft[it.key]);
                    const semTrava = missing.has(it.key);
                    const def = defaultForItem(it);
                    const overridden = on !== def;
                    const risk = RISK_META[it.riskLevel];
                    return (
                      <label key={it.key} className={"tp-acc" + (idx === 0 ? " row1" : "")}>
                        <span className="lab">
                          <span className="hdr">
                            <b>{it.label}</b>
                            <span className={"tag " + risk.cls}>{risk.label}</span>
                            {overridden && <span className="tag teal">Alterado</span>}
                            {it.sellerVisible === false && <span className="tag">Interno</span>}
                            {it.backendEnforced === false && <span className="flag">Sem trava no backend</span>}
                            {semTrava && it.backendEnforced !== false && <span className="flag">Sem trava no backend</span>}
                          </span>
                          <small>{it.description}</small>
                          <small className="tp-acc-meta">
                            chave: {it.key}
                            {it.requiresModule ? ` · módulo: ${it.requiresModule}` : ""}
                            {` · padrão do cargo: ${def ? "ligado" : "desligado"}`}
                          </small>
                        </span>
                        <span className="tp-acc-ctl">
                          {overridden && !dis && (
                            <button type="button" className="tp-resetbtn"
                              onClick={(e) => { e.preventDefault(); setAccessDraft((c) => ({ ...c, [it.key]: def })); setPresetKey(""); }}>
                              padrão
                            </button>
                          )}
      ...
      <section className="tp-sec">
        ...
        <span className="lbl" style={{ fontSize: "var(--hbx-font-min)", fontWeight: 700 }}>Canais obrigatórios no resultado</span>
        <div className="tp-grid2">
