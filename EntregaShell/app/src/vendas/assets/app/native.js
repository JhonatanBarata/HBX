/* ==========================================================================
   NATIVE — a ponte com o Kotlin, a COPIA DO VENDAS.

   🔴 POR QUE ESTE ARQUIVO EXISTE, se o Gradle ja mesclava o de `main/`.
   O `main/assets/app/native.js` e o do app ANTIGO e serve os dois flavors por
   heranca. Ele esta 86 linhas atras do de `logistica/` — e as linhas que
   faltam sao justamente as que a casca 2.0 precisa:
     · `whatsapp()` sem fallback web  — o toque na bancada (e em qualquer
       WebView sem a ponte nativa) fica indistinguivel de BOTAO MORTO, e este
       app termina toda conversa em WhatsApp;
     · `modoNavegacao()` ausente      — feature-detect que o APK velho nao tem;
     · `navigate()` sem `arrastarModulo` — o arrastar entre modulos morria
       CALADO (o `context` do app.js nunca e setado na casca 2.0);
     · `data-luz` nao espelhado       — a pele 2.0 le `data-luz`, a velha le
       `data-theme`; sem o espelho o tema tem dois donos e um dia discordam;
     · a lista do "que nao pega" no arrastar so tinha nomes do front ANTIGO
       (`.bottom-nav`, `.sheet-wrap`…), que aparecem ZERO vez na casca 2.0.
   Herdar o de `main/` era herdar cinco defeitos mudos de uma vez.

   🔴 E POR QUE NAO SE MEXE NO DE `main/`: ele e COMPARTILHADO (o app antigo
   ainda o carrega). Copia por flavor e o preco de nao quebrar o vizinho — o
   `logistica/` ja pagou esse preco em 07/08, e este arquivo nasce DELE, nao
   do `main/`, exatamente pra nao repetir o atraso de 86 linhas.

   🔴 A UNICA DIVERGENCIA DELIBERADA em relacao ao de `logistica/` e a FRASE
   DE RECUSA da `HBX.api` ("Abra esta tela pelo HBX Vendas."): copiada crua,
   ela mandaria o VENDEDOR abrir o app do MOTORISTA. O `humano()` da ponte casa
   com ela por regex (/Abra esta tela pelo HBX/), entao o prefixo tem que ficar.
   ========================================================================== */
(function () {
  "use strict";
  ...
  const HBX = {
    api(path, options) {
      if (!bridge || typeof bridge.request !== "function") {
        return Promise.reject(new Error("Abra esta tela pelo HBX Vendas."));
      }
      const opts = options || {};
