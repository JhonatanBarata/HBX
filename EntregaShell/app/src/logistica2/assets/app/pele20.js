...
function nav(ativo){
  const it=[['caderneta','Caderneta','note'],['clientes','Clientes','users'],['rota','Rota','map'],
            ['produtos','Produtos','box'],['chat','Chat','chat'],['ajustes','Ajustes','gear']];
  return `<nav class="nav">${it.map(([k,l,i])=>
...
function transmux(estado){
  ...
  return `<div class="transmux">${sat(c.esq,'esq')}
    <span class="tmx-main">
      <button data-estado="${c.main.acao}" aria-label="${c.main.rotulo}">${ic(c.main.glifo,34)}</button>
...
/* ==========================================================================
   DADOS — o QUE a tela mostra, separado de COMO ela mostra.
   ==========================================================================
   Até aqui cada template carregava os próprios literais ("João da Silva",
   "R$ 336,00", "Água 20L 128 un."). Isso deixava o mock lindo e o app CEGO: a
   pele foi pro aparelho com a exibição ligada e a fiação não, porque não havia
   por onde o dado de verdade entrar.

   🔴 O ACORDO QUE MANTÉM A REGRA DO DONO ("o html gerado tem que ser idêntico"):
   os valores daqui são EXATAMENTE os literais que estavam dentro dos templates,
   movidos, não reescritos. Sem ninguém injetar nada, o mock renderiza igualzinho
   — e é isso que `node scripts/pele20-conferir.js` continua medindo, 66/66.

   No aparelho, `window.HBX20.usarDados('produtos', {...})` troca UMA seção por
   dado real. Seção não trocada continua com o dado do mock e AVISA no console:
   enfeite silencioso já enganou uma vez nesta frente, não engana de novo.
   ========================================================================== */
const DADOS_MOCK={
  rota:{
    estado:'rodando',
    paradas:[
      ...
      {n:5,hora:'11:30',nome:'Bar do Zé',rua:'R. dos Otonis, 317',bairro:'Jabaquara • SP',tags:[['20L x3','blue'],['Vasilhame']],marcado:'63,00',pill:['Pendente','amber','clock'],perna:'2,1 km · 9 min'},
    ...
    kpi:{paradas:'12',entregues:'6',saldo:'184,00',dinheiro:'132,00',pix:'52,00'},
    progresso:{feitas:'3',total:'14',marcado:'336,00',pct:'21%'},
    filtro:{fila:'8',entregue:'6'},
    creditos:{saldo:'240',debita:'Iniciar debita 12'},
    resumo:{produtos:'20',marcado:'336,00'},
  },
  produtos:{
    categorias:['Todos','Água','Vasilhames','Acessórios','Kits'],
    lista:[
      {nome:'Água 20L',estoque:'128',preco:'18,00',cor:'azul'},
      {nome:'Água 10L',estoque:'86',preco:'12,00',cor:'azul'},
      {nome:'Vasilhame',estoque:'64',preco:'42,00',cor:'azul-escuro'},
      {nome:'Chip dia',estoque:'210',preco:'2,00',cor:'lima'},
      {nome:'Caixa térmica',estoque:'18',preco:'85,00',cor:'cinza'},
      {nome:'Kit entrega',estoque:'32',preco:'35,00',cor:'azul'},
      {nome:'Água c/ gás 1,5L',estoque:'54',preco:'24,00',cor:'azul'},
      {nome:'Suporte de galão',estoque:'12',preco:'69,00',cor:'cinza'},
    ],
    resumo:{ativos:'8',baixo:'2',valor:'194,00'},
  },
  clientes:{
    subtitulo:'14 na rota de hoje',
    dias:['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'],
    diaAtivo:'Qua',
    lista:[
      {ini:'JS',nome:'João da Silva',end:'R. das Palmeiras, 145 • Santo Amaro',dia:'Chip dia',valor:'42,00',lime:0,alerta:''},
      {ini:'MB',nome:'Mercadinho Bom Preço',end:'Av. João Dias, 890 • Brooklin',dia:'',valor:'84,00',lime:0,alerta:''},
      {ini:'MA',nome:'Maria Aparecida',end:'R. Sargento Silva Nunes, 72 • Moema',dia:'Chip dia',valor:'21,00',lime:1,alerta:''},
      {ini:'PN',nome:'Padaria Pão Nosso',end:'Av. Ibirapuera, 2331 • Moema',dia:'',valor:'42,00',lime:0,alerta:''},
      {ini:'LY',nome:'Larissa Ypê',end:'Rua 3a, 1354 • Jd. Ypê',dia:'',valor:'',lime:0,alerta:'sem número'},
      {ini:'BZ',nome:'Bar do Zé',end:'R. dos Otonis, 317 • Jabaquara',dia:'',valor:'63,00',lime:0,alerta:''},
      {ini:'ME',nome:'Mercado Estrela',end:'R. Aracanguá, 210 • Jabaquara',dia:'',valor:'84,00',lime:1,alerta:''},
      {ini:'QB',nome:'Quitanda do Bairro',end:'R. das Orquídeas, 55 • Campo Belo',dia:'Chip dia',valor:'42,00',lime:0,alerta:''},
      {ini:'DC',nome:'Depósito Central',end:'R. Dr. Jesuíno Maciel, 980 • Santo Amaro',dia:'',valor:'126,00',lime:0,alerta:''},
    ],
    resumo:{total:'128',semEndereco:'3',marcado:'336,00'},
  },
  // A caderneta mostra as MESMAS paradas do dia (por isso lê `rota.paradas`) —
  // eram 5 cópias literais aqui, palavra por palavra iguais às da rota. Dado
  // repetido é dado que um dia diverge; a caderneta só corta em quantas cabem.
  caderneta:{
    kpi:{paradas:'12',entregues:'6',saldo:'184,00'},
    quantasParadas:5,
    selo:'Tudo certo!',
    formas:[
      ...
      {nome:'Caderneta',valor:'68,00',icone:'note',cor:'var(--amber)'},
    ],
    total:'336,00',
...
/* Cópia rasa: `usarDados` troca a seção INTEIRA, então raso basta — e o
   DADOS_MOCK original fica intocado pra conferência poder comparar sempre
   contra o mesmo ponto de partida. */
let D=Object.assign({},DADOS_MOCK);
const secoesReais=new Set();
/** Troca UMA seção por dado de verdade. Seção desconhecida é ERRO ALTO: nome
 *  errado viraria tela em branco silenciosa, que é o pior desfecho possível. */
function usarDados(secao,valor){
  if(!(secao in DADOS_MOCK)) throw new Error('[pele20] seção de dados desconhecida: '+secao);
  D[secao]=valor; secoesReais.add(secao);
}
/** Volta TUDO pro dado do mock (é o que o conferidor usa entre uma tela e outra). */
function dadosDoMock(){ D=Object.assign({},DADOS_MOCK); secoesReais.clear(); }
/** Quais seções ainda são enfeite. Barulhento no console, invisível na tela. */
function secoesDeMock(){ return Object.keys(DADOS_MOCK).filter(k=>!secoesReais.has(k)); }
...
function listaParadas(comPerna){
  return D.rota.paradas.map(p=>`${comPerna&&p.perna?`<div class="perna"><span>${p.perna.startsWith('sem')?p.perna:'↓ '+p.perna}</span></div>`:''}${stop(p)}`).join('');
}
...
T.rota={nome:'Rota do dia (7 estados)',grupo:'Rota',render(){
  ...
  const k=D.rota.kpi, pr=D.rota.progresso, ft=D.rota.filtro, cr=D.rota.creditos, rs=D.rota.resumo;
  return shellRota(`
  <div class="kpis">
    <div class="kpi"><span style="color:var(--lime)">${ic('route',20)}</span><span><b class="v">${k.paradas}</b><span class="l">paradas</span></span></div>
    <div class="kpi"><span style="color:var(--lime)">${ic('check',20)}</span><span><b class="v">${emCurso?k.entregues:0}</b><span class="l">entregues</span></span></div>
    <div class="kpi money"><span class="l">Saldo</span><b class="v">R$ ${k.saldo}</b><span class="go">${ic('chev',15)}</span></div>
    <div class="kpi split">
      <span class="ln"><span style="color:var(--lime)">${ic('cash',16)}</span><span><span class="t" style="color:var(--lime)">Dinheiro</span><span class="m">R$ ${k.dinheiro}</span></span></span>
      <span class="ln"><span style="color:var(--blue-l)">${ic('pix',16)}</span><span><span class="t" style="color:var(--blue-l)">Pix</span><span class="m">R$ ${k.pix}</span></span></span>
    </div>
  </div>
  ...
  ${emCurso?`<div class="dia-bar"><small>${pr.feitas} de ${pr.total}</small><span class="trilho"><i style="width:${pr.pct}"></i></span><b>R$ ${pr.marcado}</b><small>marcado</small></div>`:''}
  ${emCurso?`<div class="filtro">
      <button class="on">Fila <b>${ft.fila}</b></button><button>Entregue <b>${ft.entregue}</b></button></div>`:''}
  ${montada||e==='montar'?`<div class="creditos">${ic('card',17)}
      <span><b class="v">${cr.saldo}</b> <small>créditos hoje</small></span>
      <span class="debita">${montada?cr.debita:'monte a rota pra saber'}</span></div>`:''}
  <div class="stops">${listaParadas(emCurso)}</div>
  <div class="sum">
    <span class="c"><span style="color:var(--lime)">${ic('box',17)}</span><span><b>${rs.produtos}</b><small>produtos</small></span></span>
    <span class="c"><span style="color:var(--ink-2)">${ic('receipt',17)}</span><span><b>R$ ${rs.marcado}</b><small>marcado</small></span></span>
    <span class="c"><span style="color:var(--lime)">${ic('check',17)}</span><span><b>${emCurso?k.entregues:0}</b><small>entregas</small></span></span>
  ...
  <div class="gps">
    ...
    <div class="gps-manobra chegou">
      <div class="cima">
        ...
        <span><b class="dist">Você chegou</b>
          <span class="verbo">Mercado São Judas</span></span>
      </div>
      <div class="baixo">R. São Judas, 142 · ${ic('gps',13)} GPS ±6 m, você está na porta</div>
    </div>
    <div class="gps-lado" style="bottom:118px">
      <button aria-label="Recentralizar">${ic('target',18)}</button>
    </div>
    <div class="gps-rodape">
      <div class="parada">${ic('route',14)} Parada <b>3 de 8</b> · faltam <b>5 paradas</b> · <b>6,4 km</b></div>
      <button class="act go full" style="justify-content:center">${ic('check',20)}<b>Registrar entrega</b></button>
    </div>
  </div>
</div>`;
...
<div class="body flush" style="overflow:hidden;padding:0">
  <div class="gps">
    ...
    <div class="gps-manobra">
      <div class="cima">
        <span class="seta">${ic('curvaDireita',26)}</span>
        <span><b class="dist">240 m</b><span class="verbo">Vire à direita</span></span>
      </div>
      <div class="baixo">R. São Judas · depois, siga em frente por 1,2 km</div>
    </div>

    <!-- bússola: só existe porque o mapa gira pelo rumo -->
    <div class="gps-bussola">
      <svg viewBox="0 0 24 24" width="15" height="15"><path d="M12 3.5 L16 13 L12 10.6 L8 13 Z" fill="#ff8b85"/></svg>N
    </div>
    <div class="gps-vel"><b>38</b><small>km/h</small></div>
    <div class="gps-lado">
    ...
    <div class="gps-rodape">
      <div class="parada">${ic('route',14)} Parada <b>3 de 8</b> · <b>Mercado São Judas</b></div>
      <div class="linha">
        <span class="n destaque"><b>12:26</b><small>chegada</small></span>
        <span class="n"><b>45 min</b><small>restante</small></span>
        <span class="n"><b>8,2 km</b><small>distância</small></span>
        <button class="sair" data-ir="rota">Encerrar</button>
      </div>
    </div>
  </div>
</div>`;
...
/* 4 — FOLHA DA VENDA ------------------------------------------------------ */
T.venda={nome:'Folha da venda',grupo:'Rota',render(){return `${status}
${hdr({})}
  ...
  <div class="sheet">
    ...
    <div class="box">
      <div style="display:flex;gap:11px;align-items:flex-start">
        ...
        <span style="flex:1"><span class="box-t">Água Mineral HBX</span>
          <span class="tags" style="margin-top:6px"><b class="tag blue">20L x1</b><b class="tag">Vasilhame</b><b class="tag lime">Chip dia</b></span></span>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px">
        <span style="font-size:12.5px;color:var(--ink-2)">Conta do item</span>
        <b style="font-size:18px;color:var(--lime);font-weight:500">R$ 21,00</b></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px">
      <div class="box" style="display:flex;align-items:center;justify-content:space-between;margin:0">
        <span><span class="box-s">Conta da chegada</span><b style="display:block;font-size:15px;color:var(--lime);margin-top:2px">R$ 21,00</b></span>
        <span style="color:var(--lime)">${ic('check',19)}</span></div>
      <div class="box" style="display:flex;align-items:center;justify-content:space-between;margin:0">
        <span><span class="box-s">Lançamento na caderneta</span><b style="display:block;font-size:15px;color:var(--lime);margin-top:2px">R$ 21,00</b></span>
    ...
    <div class="box">
      ...
      <div class="pays">
        <button class="pay sel"><span style="color:var(--lime)">${ic('cash',21)}</span><span><b>Dinheiro</b><small>Pagamento em espécie</small></span><i class="ok">${ic('check',15)}</i></button>
        <button class="pay blue"><span style="color:var(--blue-l)">${ic('pix',21)}</span><span><b>Pix</b><small>Transferência instantânea</small></span></button>
        <button class="pay"><span style="color:var(--ink-2)">${ic('card',21)}</span><span><b>Cartão</b><small>Débito ou crédito</small></span></button>
        <button class="pay"><span style="color:var(--ink-2)">${ic('note',21)}</span><span><b>Caderneta</b><small>Lançar na caderneta</small></span></button>
      </div>
    </div>
    <div class="box">
      ...
      <div class="rowline"><span>Recebido hoje</span><b>R$ 21,00</b></div>
      <div class="rowline"><span>Vai para caderneta</span><b>R$ 21,00</b></div>
    </div>
    <div class="foot2">
      <button class="act go" style="justify-content:center">${ic('check',19)}<b>Confirmar venda</b></button>
...
<div class="body">
  <div class="kpis">
    <div class="kpi"><span style="color:var(--lime)">${ic('route',20)}</span><span><b class="v">${D.caderneta.kpi.paradas}</b><span class="l">paradas</span></span></div>
    <div class="kpi"><span style="color:var(--lime)">${ic('check',20)}</span><span><b class="v">${D.caderneta.kpi.entregues}</b><span class="l">entregues</span></span></div>
    <div class="kpi money"><span class="l">Saldo</span><b class="v">R$ ${D.caderneta.kpi.saldo}</b><span class="go">${ic('chev',15)}</span></div>
  </div>
  <div class="bar"><span class="t">${ic('list',17)} Paradas de hoje</span>
    <button class="ghost" data-ir="mapa" data-acao="ver-mapa">${ic('map',15)} Ver mapa</button></div>
  <div class="stops">
    ${D.rota.paradas.slice(0,D.caderneta.quantasParadas).map(p=>stop(p)).join('')}
  </div>
</div>
<div class="sheet" style="max-height:52%">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
    ...
    <span class="pill mute">${D.caderneta.selo} ${ic('check',14)}</span>
  </div>
  <div class="forms">
    ${D.caderneta.formas.map(f=>`<div class="form-c"><span style="color:${f.cor}">${ic(f.icone,19)}</span><small>${f.nome}</small><b>R$ ${f.valor}</b></div>`).join('')}
    <div class="form-c total"><small style="margin-top:0">Total</small><b>R$ ${D.caderneta.total}</b></div>
  </div>
  <div class="sum">
    <span class="c"><span style="color:var(--ink-2)">${ic('users',17)}</span><span><b>${D.caderneta.resumo.clientes}</b><small>clientes</small></span></span>
    <span class="c"><span style="color:var(--ink-2)">${ic('box',17)}</span><span><b>${D.caderneta.resumo.produtos}</b><small>produtos</small></span></span>
    <span class="c"><span style="color:var(--ink-2)">${ic('receipt',17)}</span><span><b>R$ ${D.caderneta.resumo.marcado}</b><small>marcado</small></span></span>
  </div>
  <div class="acts">
    <button class="act go" style="justify-content:center" data-acao="caderneta-fechar">${ic('check',19)}<b>Fechar o dia</b></button>
    <button class="act" style="justify-content:center" data-ir="semana" data-acao="caderneta-detalhes">${ic('chart',17)}<b>Ver detalhes</b></button>
  </div>
</div>
${nav('caderneta')}`;}};
...
T.produtos={nome:'Produtos',grupo:'Cadastro',render(){
  ...
  const p=(nome,est,preco,cor)=>`<div class="prod" data-acao="produto-abrir" data-valor="${nome}">
...
<div class="body">
  ...
  <div class="searchrow">
    <label class="search">${ic('search',17)}<input placeholder="Buscar produto"></label>
    <button class="filt">${ic('sliders',18)}</button></div>
  <div class="chips">
    ${D.produtos.categorias.map((cat,i)=>`<button class="chip${i===0?' on':''}" data-acao="produtos-categoria" data-valor="${cat}">${cat}</button>`).join('')}</div>
  <div class="lista-card">
    ${D.produtos.lista.map(x=>p(x.nome,x.estoque,x.preco,x.cor)).join('')}
  </div>
  <div class="sum">
    <span class="c"><span style="color:var(--lime)">${ic('box',17)}</span><span><b>${D.produtos.resumo.ativos}</b><small>produtos ativos</small></span></span>
    <span class="c"><span style="color:var(--amber)">${ic('alert',17)}</span><span><b>${D.produtos.resumo.baixo}</b><small style="color:var(--amber)">estoque baixo</small></span></span>
    <span class="c"><span style="color:var(--blue-l)">${ic('cash',17)}</span><span><b>R$ ${D.produtos.resumo.valor}</b><small>valor estimado</small></span></span>
  </div>
</div>
${nav('produtos')}`;}};
...
T.clientes={nome:'Clientes',grupo:'Cadastro',render(){
  const c=(ini,nome,end,dia,valor,tomLime,alerta)=>`<div class="cli" data-acao="cliente-abrir" data-valor="${nome}">
...
<div class="body">
  <div class="screen-head"><span style="color:var(--lime)">${ic('users',30)}</span>
    <span><h2>Clientes</h2><p>${D.clientes.subtitulo}</p></span></div>
  <div class="searchrow">
    <label class="search">${ic('search',17)}<input placeholder="Buscar cliente"></label>
    <button class="filt">${ic('sliders',18)}</button></div>
  <div class="chips">
    ${D.clientes.dias.map(d=>`<button class="chip${d===D.clientes.diaAtivo?' on':''}" data-acao="clientes-dia" data-valor="${d}">${d}</button>`).join('')}</div>
  <div class="lista-card">
    ${D.clientes.lista.map(x=>c(x.ini,x.nome,x.end,x.dia,x.valor,x.lime,x.alerta)).join('')}
  </div>
  <div class="sum">
    <span class="c"><span style="color:var(--lime)">${ic('users',17)}</span><span><b>${D.clientes.resumo.total}</b><small>clientes</small></span></span>
    <span class="c"><span style="color:var(--amber)">${ic('alert',17)}</span><span><b>${D.clientes.resumo.semEndereco}</b><small style="color:var(--amber)">sem endereço</small></span></span>
    <span class="c"><span style="color:var(--lime)">${ic('cash',17)}</span><span><b>R$ ${D.clientes.resumo.marcado}</b><small>marcado hoje</small></span></span>
  </div>
</div>
${nav('clientes')}`;}};
...
${hdr({sub:titulo,voltar:'ajustes',semChat:1})}
...
/* 21 — FICHA DO PRODUTO ---------------------------------------------------- */
T.fichaproduto={nome:'Ficha do produto',grupo:'Cadastro',render(){return `${status}
${hdr({sub:'Ficha do produto',voltar:'produtos',semChat:1})}
<div class="body">
  <div class="stop-top" style="margin:2px 0 10px;grid-template-columns:auto 1fr auto">
    ...
    <span><strong style="font-size:16px">Galão 20 Litros</strong>
      <span style="font-size:11px;color:var(--ink-2)">no catálogo desde 03/2025 · 1.284 entregas</span></span>
    <span class="tag lime">ativo</span>
  </div>
  <div class="campos">
    <label class="campo"><label>Nome</label><input value="Galão 20 Litros"></label>
    <div class="dupla">
      <label class="campo"><label>Unidade</label><input value="galão" placeholder="galão, caixa, unidade"></label>
      <label class="campo"><label>Preço do catálogo</label><input value="R$ 11,00"></label>
    </div>
    <label class="campo"><label>Estoque</label><input value="128"></label>
  </div>
  <div class="banner pausa" style="margin-top:9px">${ic('alert',15)}
...
${hdr({sub:'Passeio',voltar:'ajustes'})}
...
${hdr({sub:'Registrando o caminho',voltar:'rota'})}
<div class="body com-dock">
  <div class="gravando">
...
T.ficha={nome:'Ficha do cliente',grupo:'Cadastro',render(){
  const dia=(l,n,on)=>`<button class="${on?'on':''}"><b>${l}</b><small>${n}</small></button>`;
  return `${status}
${hdr({sub:'Ficha do cliente',voltar:'clientes',semChat:1})}
<div class="body">
  <div class="stop-top" style="margin:2px 0 10px;grid-template-columns:auto 1fr auto">
    <span class="ava lime">LY</span>
    <span><strong style="font-size:16px">Larissa Ypê</strong>
      <span style="font-size:11px;color:var(--ink-2)">cliente desde 03/2025 · 42 entregas</span></span>
    <span class="tag" style="border-color:rgba(245,165,36,.55);color:var(--amber)">sem número</span>
  </div>

  <div class="grupo">Quem é</div>
  <div class="campos">
    <label class="campo"><label>Nome</label><input value="Larissa Ypê"></label>
    <div class="dupla">
      <label class="campo"><label>Telefone / WhatsApp</label><input value="(19) 99812-4477"></label>
      <label class="campo"><label>CPF</label><input placeholder="000.000.000-00"></label>
    </div>
  </div>

  <div class="grupo">Onde é</div>
  <div class="campos">
    <label class="campo"><label>CEP</label><input value="13503-210"></label>
    <div class="tripla">
      <label class="campo"><label>Rua</label><input value="Rua 3a"></label>
      <label class="campo erro"><label>Número <span class="pend">· pendente</span></label><input placeholder="—">
        <span class="dica">sem número o pino não fecha</span></label>
      <label class="campo"><label>Bairro</label><input value="Jd. Ypê"></label>
    </div>
    <label class="campo"><label>Observações da porta</label>
      <textarea>Portão azul · deixar na área · cachorro solto</textarea></label>
  </div>

  <div class="grupo">Dias de entrega</div>
  <div class="dias">
    ${dia('SEG','',0)}${dia('TER','',1)}${dia('QUA','',0)}${dia('QUI','',1)}
    ${dia('SEX','',0)}${dia('SÁB','',0)}${dia('DOM','',0)}
  </div>
  ...
  <div class="cartao-lista" style="padding:0 11px">
    <div class="item-linha"><span class="ava" style="width:32px;height:32px">${ic('gallon',16)}</span>
      <span><strong>Galão 20 Litros</strong><span>2 por entrega · R$ 11,00 (catálogo)</span></span>
      <span style="color:var(--ink-3)">${ic('chev',15)}</span></div>
    <div class="item-linha"><span class="ava" style="width:32px;height:32px">${ic('box',16)}</span>
      <span><strong>Água c/ gás 1,5L</strong><span>1 por entrega · <b style="color:var(--lime)">R$ 22,00 só pra ela</b></span></span>
      <span style="color:var(--ink-3)">${ic('chev',15)}</span></div>
  </div>
  ...
  <div class="sheet">
    ...
    <div class="sheet-head">
      <div><h2>${naoEntregue?'Não entregue':'Chegada'}</h2>
        <p>${naoEntregue?'Diga o que aconteceu':'Parada 3 · Maria Aparecida'}</p></div>
      <button class="round sm" data-ir="rota">${ic('close',16)}</button></div>
    ...
    ${naoEntregue?`
      <div class="box">
        ...
        <div class="motivos">
          <button class="motivo on"><span class="bola"></span>Ninguém atendeu</button>
          <button class="motivo"><span class="bola"></span>Cliente pediu pra voltar depois</button>
          <button class="motivo"><span class="bola"></span>Endereço não encontrado</button>
          <button class="motivo"><span class="bola"></span>Sem troco / não tinha o dinheiro</button>
          <button class="motivo"><span class="bola"></span>Estabelecimento fechado</button>
        </div>
      </div>
      <div class="box">
        <div class="box-t">Comprovante</div>
        <div class="box-s">Uma foto evita discussão depois. Opcional.</div>
        <div class="foto"><span class="quadro">${ic('photo',24)}</span>
          <span><strong>Tirar foto da porta</strong><span>fica guardada com a entrega</span></span></div>
      </div>
      <div class="foot2">
        <button class="act go" style="justify-content:center">${ic('check',19)}<b>Registrar</b></button>
        <button class="act" style="justify-content:center" data-ir="folha">${ic('back',17)}<b>Voltar</b></button>
      </div>`
    :`
      <div class="box" style="display:flex;align-items:center;gap:10px">
        <span class="num lime" style="width:36px;height:36px">3</span>
        <span style="flex:1"><span class="box-t">Maria Aparecida</span>
          <span class="box-s">R. Sargento Silva Nunes, 72 • Moema</span></span>
        <span class="pill lime">${ic('check',14)}Chegou</span>
      </div>
      <div class="banner alerta" style="margin-top:8px">${ic('alert',15)}
        <span>Portão azul · deixar na área · <b>cachorro solto</b></span></div>

      <div class="box">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <span class="box-t">Conferir o que saiu</span>
          <button class="ghost">${ic('plus',13)} Produto</button></div>
        <div class="item-linha"><span class="thumb" style="width:40px;height:40px;flex:0 0 40px">${ic('gallon',20)}</span>
          <span><strong>Galão 20 Litros</strong><span>previsto 2 · R$ 11,00 cada</span></span>
          <span class="passo"><button>−</button><b>2</b><button>+</button></span></div>
        <div class="item-linha"><span class="thumb" style="width:40px;height:40px;flex:0 0 40px">${ic('box',20)}</span>
          <span><strong>Água c/ gás 1,5L</strong><span>previsto 1 · R$ 24,00 cada</span></span>
          <span class="passo"><button>−</button><b>0</b><button>+</button></span></div>
      </div>

      <div class="box">
        <div class="rowline"><span>Anterior</span><b style="color:var(--amber)">R$ 21,00</b></div>
        <div class="rowline"><span>Venda de hoje</span><b>R$ 22,00</b></div>
        <div class="rowline"><span style="font-size:16px;color:var(--ink)">Total</span>
          <b style="font-size:19px">R$ 43,00</b></div>
      </div>

      <div class="box">
        <div class="box-t">Como pagou</div>
        <div class="pays">
          <button class="pay sel"><span style="color:var(--lime)">${ic('cash',21)}</span><span><b>Dinheiro</b><small>em espécie</small></span><i class="ok">${ic('check',15)}</i></button>
          <button class="pay blue"><span style="color:var(--blue-l)">${ic('pix',21)}</span><span><b>Pix</b><small>na hora</small></span></button>
          <button class="pay"><span style="color:var(--ink-2)">${ic('card',21)}</span><span><b>Cartão</b><small>débito ou crédito</small></span></button>
          <button class="pay"><span style="color:var(--amber)">${ic('note',21)}</span><span><b>Marcar</b><small>vai pra caderneta</small></span></button>
        </div>
      </div>
...
T.gerenciador={nome:'Gerenciador de rota',grupo:'Rota',render(){
  ...
  return `${status}
${hdr({sub:'Montar o dia',voltar:'rota'})}
<div class="body com-dock">
...
function avisar(tipo){
  ...
  const cfg=AVISOS[tipo]||AVISOS.recado;
  camada.querySelector('.aviso')?.remove();
  clearTimeout(avisoTimer);
  const el=document.createElement('div');
  el.className='aviso '+cfg.cls;
  el.innerHTML=`<span class="ico">${ic(cfg.ico,17)}</span>
