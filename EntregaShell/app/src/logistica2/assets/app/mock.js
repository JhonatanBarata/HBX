...
function pintar(animar,dir){
  ...
  if(animar && antiga && tr!=='nenhuma'){
    ...
    /* 🔴 REPINTAR NÃO PODE ROLAR A FOLHA PRA CIMA. A folha (`.sheet`) é o
       único painel que ROLA por dentro, e escolher a forma de pagamento fica
       no pé dela: sem isto, cada toque jogava o dedo de volta pro topo e o
       botão de confirmar fugia da tela. Vale pra qualquer repinte do seam,
       não só o do pagamento — por isso mora aqui e não em quem chamou. */
    if(antiga&&!animar){
      const de=antiga.querySelector('.sheet'), para=nova.querySelector('.sheet');
      if(de&&para&&de.scrollTop) para.scrollTop=de.scrollTop;
    }
