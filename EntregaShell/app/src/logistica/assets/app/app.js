(function () {
  ...
  function clientEditorModal(isNew) {
    ...
    return `<div class="modal-wrap" data-action="close-modal"><section class="modal client-edit-modal"><div class="sheet-head ${pending.length ? "client-head-pending" : ""}"><div class="avatar">${icon("users", 18)}</div><div><h2>${isNew ? "Novo cliente" : "Editar cliente"}</h2><p class="subtitle">${H.escape(isNew ? "Preencha os dados do cliente" : (client.nome || client.name || "Cliente"))}</p></div><button class="close" data-action="close-modal">${icon("close", 18)}</button></div>${pending.includes("Dup") ? `<div class="client-duplicate-warning">Endereço duplicado: confira antes de salvar.</div>` : ""}<div class="client-editor-body">${customerForm}${isNew ? "" : productPart}</div></section></div>`;
  }
  function modal() {
