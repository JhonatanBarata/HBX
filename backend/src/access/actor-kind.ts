...
//   dono     → ADMIN que vê cobrança (canViewBilling !== false).
//   gerente  → ADMIN sem cobrança (canViewBilling === false).
//   vendedor → USER (funcionário comum).
//   user     → qualquer outro (inclui USERMASTER SEM a flag — anti-spoof do
//              roles.guard.ts:25; role de string não promove a master).
...
export function resolveActorKind(user: ActorKindUserLike): ActorKind {
  ...
  if (role === 'ADMIN') return user?.canViewBilling === false ? 'gerente' : 'dono';
