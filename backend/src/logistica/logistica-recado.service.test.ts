...
test('puxar: entrega marca ✓✓ e o mesmo recado não volta no próximo pull', async () => {
  const h = harness({
  ...
  assert.equal(primeira[0].estado, 'no_aparelho', 'puxou = chegou no aparelho');
  const segunda = await h.service.puxar(1, 7);
  assert.equal(segunda.length, 0, 'recado entregue não volta no pull');
});
...
test('responder: a resposta do motorista nasce entregue e vista (veio do aparelho)', async () => {
  const h = harness();
  const row = await h.service.responder(1, 7, 'Cliente não atende');
  assert.equal(row.origem, 'motorista');
  assert.equal(row.estado, 'visto', 'quem escreveu já leu — não se cobra ✓✓ de si mesmo');
});
