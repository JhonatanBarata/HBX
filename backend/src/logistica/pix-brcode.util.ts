// ================================================================
// S2 COBRANÇA-WHATS (11/07) — BR Code Pix ESTÁTICO no backend.
//
// PORT 1:1 de frontend/src/app/entrega/pix-brcode.ts (LOGÍSTICA F1, a fonte
// canônica da casa). O backend NÃO importa código do frontend (pacotes
// separados), então o algoritmo é espelhado aqui SEM mudança de comportamento —
// mesma entrada, mesmo payload, mesmo CRC. Mudou lá? Espelhar aqui.
//
// O payload EMV®-QRCPS do Pix é padrão ABERTO (manual do BCB): chave do
// RECEBEDOR (o tenant, LogisticaConfig.pixChave/pixNome/pixCidade) + valor +
// CRC16. Nada passa por API/gateway — dinheiro cai DIRETO na conta do tenant,
// taxa ZERO. MercadoPago NÃO entra aqui (regra do S2: não tocar MP).
//
...
export interface PixBrCodeInput {
  ...
  chave: string;
  /** Valor em R$. null/0 → código sem valor (o pagador digita). */
  valor?: number | null;
