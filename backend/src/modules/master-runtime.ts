...
async function ensureMasterBillingRuntimeSchemaUncached(prisma: PrismaService) {
  ...
  // Backfill idempotente do estado unico (mesma logica da migracao
  // 20260610_company_unified_status): linhas ja convertidas tem
  // statusChangedAt preenchido e nao sao tocadas de novo.
  await prisma.$executeRawUnsafe(`
    UPDATE "Company" SET
      "status" = CASE
