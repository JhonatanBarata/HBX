...
async function ensureMasterBillingRuntimeSchemaUncached(prisma: PrismaService) {
  ...
  await prisma.$executeRawUnsafe(`
    ALTER TABLE "Company"
    ADD COLUMN IF NOT EXISTS "status" TEXT NOT NULL DEFAULT 'pending_checkout'
  `);
  ...
  await prisma.$executeRawUnsafe(`
    ALTER TABLE "Company"
    ADD COLUMN IF NOT EXISTS "statusChangedByUserId" INTEGER
  `);

  await prisma.$executeRawUnsafe(`
    ALTER TABLE "Company"
    ADD COLUMN IF NOT EXISTS "courtesyEndsAt" TIMESTAMP(3)
  `);

  await prisma.$executeRawUnsafe(`
    ALTER TABLE "Company"
    ADD COLUMN IF NOT EXISTS "courtesyReason" TEXT
  `);

  // Pos-DROP (PR-002): o backfill legado e os ensures de billingExempt*
  // morreram junto com as colunas. Cortesia sem motivo registrado ganha o
  // texto padrao de migracao (idempotente).
  await prisma.$executeRawUnsafe(`
    UPDATE "Company"
    SET "courtesyReason" = 'Migração: liberação manual/sem leitura financeira — revisar'
