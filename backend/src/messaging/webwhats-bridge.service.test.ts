...
// PR05072026 (timing humano): sendText com typingDelayMs repassa `delay`+`presence`
// ao motor (padrão Evolution API — o composing acontece DENTRO do /message/sendText,
// não é uma chamada de presence separada) e estende o timeout HTTP para cobrir o delay.
test('sendText com typingDelayMs repassa delay+presence composing ao motor e amplia o timeout', async () => {
  const previousUrl = process.env.WHATSAPP_MODAL_INTERNAL_URL;
  const previousKey = process.env.WHATSAPP_MODAL_API_KEY;
  process.env.WHATSAPP_MODAL_INTERNAL_URL = 'http://webwhats.test';
  process.env.WHATSAPP_MODAL_API_KEY = 'test-key';
  ...
  const calls = {
    path: null as string | null,
    data: null as Record<string, unknown> | null,
    timeoutOverrideMs: null as number | undefined | null,
  };
  const prisma = {
    company: {
      findUnique: async () => ({
  ...
  (service as any).requestRead = async (input: any) => {
    calls.path = input.path;
    calls.data = input.data;
    calls.timeoutOverrideMs = input.timeoutOverrideMs;
    return { key: { id: 'MSG-2' } };
  };

  try {
    await service.sendText(10, {
      to: '+5511999990000',
      text: 'Boa! Posso te ligar rapidinho?',
      typingDelayMs: 4200,
    });
    ...
    // timeout precisa cobrir o delay inteiro + margem (a resposta HTTP só chega
    // depois que o motor termina de digitar e enviar).
    assert.equal(calls.timeoutOverrideMs, 4200 + 15000);
  } finally {
    if (previousUrl === undefined) delete process.env.WHATSAPP_MODAL_INTERNAL_URL;
    else process.env.WHATSAPP_MODAL_INTERNAL_URL = previousUrl;
    if (previousKey === undefined) delete process.env.WHATSAPP_MODAL_API_KEY;
...
test('sendText sem typingDelayMs mantem o payload atual (sem delay/presence, sem override de timeout)', async () => {
  const previousUrl = process.env.WHATSAPP_MODAL_INTERNAL_URL;
  const previousKey = process.env.WHATSAPP_MODAL_API_KEY;
  process.env.WHATSAPP_MODAL_INTERNAL_URL = 'http://webwhats.test';
  process.env.WHATSAPP_MODAL_API_KEY = 'test-key';
  ...
  try {
    await service.sendText(10, { to: '+5511999990000', text: 'Oi' });

    assert.deepEqual(calls.data, { number: '+5511999990000', text: 'Oi' });
    assert.equal(calls.timeoutOverrideMs, undefined);
  } finally {
    if (previousUrl === undefined) delete process.env.WHATSAPP_MODAL_INTERNAL_URL;
    else process.env.WHATSAPP_MODAL_INTERNAL_URL = previousUrl;
    if (previousKey === undefined) delete process.env.WHATSAPP_MODAL_API_KEY;
