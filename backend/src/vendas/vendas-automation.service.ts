...
export class VendasAutomationService implements OnModuleInit, OnModuleDestroy {
  ...
  // público abaixo (mesmo nome, chamado pelo controller) é um no-op benigno —
  // não cria job novo pra ninguém. A cópia legada foi REMOVIDA em 26/07 por
  // ordem do dono (campanha running de prod cancelada na mesma data); se um
  // dia precisar, o histórico do git guarda (commit do S7 `befc4802`).
