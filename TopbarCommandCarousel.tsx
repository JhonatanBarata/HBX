...
function getVisibleSlides(slides: HbxTopbarSlide[], engines: HbxTopbarEngine[]) {
  ...
  if (!engines.length || hasEngineSlide) return safeSlides;
  ...
  return [
    ...safeSlides,
    {
      ...
      kind: "engines",
      eyebrow: "FROTA HBX",
      title: `${engines.length} motores em observação`,
...
function HbxGenericSlide({
  ...
  const metrics = Array.isArray(slide.metrics) ? slide.metrics.slice(0, 7) : [];
...
function HbxEngineFleetSlide({ engines, slide }: { engines: HbxTopbarEngine[]; slide: HbxTopbarSlide }) {
...
export default function TopbarCommandCarousel({
  ...
  const visibleSlides = useMemo(() => getVisibleSlides(slides, engines), [slides, engines]);
          ...
          {visibleSlides.map((slide) => (
            <div className="hbxp-carousel__slide" key={slide.id} aria-hidden={slide.id !== visibleSlides[currentIndex]?.id}>
              {String(slide.kind || "").toLowerCase() === "engines" ? (
                <HbxEngineFleetSlide engines={engines} slide={slide} />
              ) : (
...
export const HBX_TOPBAR_COMMAND_CAROUSEL_CSS = `
  .hbxp-carousel {
    --hbxp-card-h: clamp(132px, 9.2vw, 150px);
    --hbxp-safe-x: 38px;
    position: relative;
    width: 100%;
