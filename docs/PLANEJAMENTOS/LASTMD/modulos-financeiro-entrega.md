...
- ⚠️ **1 residual PRÉ-EXISTENTE não-fatal: React #418 (hydration mismatch) no /entrega/ajustes.** NÃO é do meu trabalho — padrão SSR de tema/fullscreen/saudação que pré-data o W4 e existe app-wide (mesmo `saudacaoPorHorario` na `/logistica/config` do ERP). React recupera (re-render no cliente), página funciona 100%. Corrigi 1 fonte (a saudação `new Date()` no render → fixa "Bom dia"; publicado). Residual (tema `modeAttr` useSyncExternalStore null-SSR / fullscreen) deixado p/ pass dedicado — não vale queimar mais publishes de prod num warning cosmético. Diagnóstico certeiro exige repro local (erro não-minificado).
