...
| **L5** | ✅ **FEITA E PROVADA NO g15 POR TOQUE** (07/08, `6cde7938`). Caixa do dia por forma (Caderneta R$ 20,00 · Total R$ 20,00 · 1 venda · 1 cliente · 2 produtos), semana dia a dia (Quinta 5 vendas R$ 75 · Sexta 1 venda R$ 20) e **"Fechar o dia" criou a "Caderneta de Sexta" nas Rotas salvas**, conferido no banco. Modo caderneta OFF devolve o recado do servidor na tela, provado antes de ligar a chave. 🔴 3 defeitos achados usando: "produtos/marcado" liam a ROTA (venda de 2 galões virava "0 produtos"), o FUNDO da semana era maquete ("João da Silva" por cima do dinheiro real) e "1 vendas". ⚠️ `resumo-dia` **não serve**: é `@Admin()`+billing-owner e não está na allowlist do celular. ⬜ sem fonte: "produtos por dia" e "recebido por dia" da semana ficam VAZIOS (decisão do dono: 7 chamadas × 2 campos aditivos no `historicoDias`). ⬜ "registrar em OUTRO dia" não tem casa no mock. | caderneta resumo/finalizar |
| **L6** | ✅ **FEITA E PROVADA NO g15 POR TOQUE** (07/08, `86622ca4`). Lista real (busca + chips de dia, PULL no servidor) → ficha real (quem é / onde é / dias / o que leva, com o vínculo `cliente-produtos`) → digitei o número, marquei SEGUNDA, Salvar. **Medido no banco: `numero` null→"32", `lat/lng` → NULL (o pino morre com o endereço velho), `LogisticaPlanoEntrega.diaSemana` → 1**, e o selo "sem número" sumiu do cabeçalho. 🔴 2 defeitos meus achados na tela: o digitado sumia no repinte, e o conserto por "foto dos campos" apagou CEP/rua/bairro — rascunho nasce de TECLA. ⬜ CPF virou só leitura (o `UpdateContaDto` não grava documento — decisão do dono). ⬜ "sem endereço" do rodapé fica vazio (é conta da base inteira e o servidor só manda o total). ⚠️ o portão pegou uma CRASE dentro de comentário HTML em template literal — teria dado tela preta. | nucleo/clientes · contas · locais · telefones · dias · cliente-produtos |
| **L7** | ✅ **FEITA E PROVADA NO g15 POR TOQUE** (07/08, `7c69eb4b`). Catálogo real (nome, unidade, preço) → ficha → troquei o preço → **portão mostra o número JÁ LIDO antes de gravar** ("passa a valer R$ 9,50"); medido no banco: `Product.price` 9 → 9.5. 🔴 O portão nasceu de um acidente na tela: dedo escorregado virou "9 509,50" e o campo é texto livre (o mock não tem a moeda "estilo banco" do app velho). ⬜ falta a MÁSCARA — o portão é freio, não cura. **Sumiram da tela por não ter fonte** (lei 5): estoque do cartão (virou a unidade), chips de categoria, "estoque baixo", "valor estimado" e o campo Estoque. ⚠️ **decisão do dono:** o PATCH aceita `estoque` mas o GET não devolve — dá pra ESCREVER e não dá pra LER; `stock` e `category` já existem na tabela. | produtos · cliente-produtos |
| **L8** | ✅ **FEITA E PROVADA NO g15 POR TOQUE** (07/08, `749b37b8`). Ciclo inteiro: leio os recados da Central com hora real → **o sino zera** → escrevo e envio → minha resposta vira balão do meu lado → o **cartão do portão** aparece e o "Entendi" o derruba. Medido no banco a cada passo (`vistoEm` nos dois do escritório, `ackEm` no urgente). 🔴 **LEI NOVA: falha de rede não apaga a tela** — o seam era escrito mesmo com tudo falhando, e uma queda limparia a conversa e zeraria o sino na mão de quem está na rua; o MESMO defeito estava na caderneta e no crédito. ⚠️ **quem achou foi o portão**: `casca-conferir` acusou **60 de 66** telas por causa do sino no cabeçalho. O sino passou a ter UMA fonte (clientes/produtos cravavam `badge:3`). 2 erros meus: `recados/visto` exige a lista de ids, e "Recado de Central" (o literal do dono já estava certo). **Rotas indicadas e missões não entram — saíram no corte.** | recados (me · portao · visto · responder · entendi) |
| **L9** | ✅ **FEITA E PROVADA NO g15 POR TOQUE** (07/08, `827666e2`). Chaves reais (Avisar chegada 500 m, Modo caderneta, Versão 2.0.0-bancada); **virei o Modo caderneta na tela e o banco foi junto (t→f)**. Recarga com os pacotes reais do `/credits/me` (100/R$97 · 300/R$247 “Mais vendido” · 800/R$597) e checkout NATIVO. 🔴 **Chave sem porta não vira chave** — saíram: o grupo “Sem internet” inteiro (corte de 06/08), “Painel de créditos do dia”, “~17 dias no seu ritmo”, “Nome da empresa”. Som/voz e tema são do APARELHO, não do servidor. “Sair” ganhou confirmação. | config · credits/me · creditos/extrato |
| **L10** | 🔶 **PARCIAL — salvas FEITA E PROVADA** (07/08, `827666e2`): a “Caderneta de Sexta” do L5 aparece com dia e paradas reais e **“Abrir” gerou a rota de hoje (entregas 0→1, medido)** — o ciclo fecha: vender → fechar o dia → vira rota salva → usar amanhã. Duplicar/3-pontos/“Ordenar por” não são renderizados (corte + uma ordem só não é escolha). ⬜ **`rapida` e `gerenciador` NÃO entregues**: `rapida` é um fluxo inteiro de captura de endereço (geo busca/cep/link/reverse + anti-duplicata + criar conta + entrega avulsa), do tamanho da L6; `gerenciador` é tirar-do-dia/sanitizar/limpar-dia + reordenar. Não entrego meia porta em fluxo que cria cliente e mexe em rota. | rota-modelos |
