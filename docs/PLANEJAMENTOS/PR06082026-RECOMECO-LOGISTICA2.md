...
| F1 casca trocável | 🔶 levas 1–3 FEITAS (06/08 noite, `e92718e9`+`cfb2fc31`+`a182e603`) — portão antes×depois nasceu e julgou tudo: 12 hex→token existente · **120 hex→20 tokens de família** (`--go-*`, `--lime/blue/amber/red-*`, `--glass`, `--field`, `--white`) · colapso conservador do claro (`--lime-bg` 5/5 + 2 regras mortas). **Placar: 404→296 hex · claro 145→143 regras.** ⚠️ Leva 3b (matar declaração por análise estática) REPROVADA pelo portão — colapso por declaração exige medição COMPUTADA por seletor, não leitura de cascata. Faltam: rabo de ~296 hex (maioria 1-2 usos), colapso medido do claro, `--casca` no injetor + casca de prova. | casca de prova trocando o ambiente |
