# S8 — Destravar o robô: SÓ config do Admin + WhatsApp liberam (regra do dono 26/07)
...
Tradução operacional — travas de ATIVAÇÃO do robô passam a ser SOMENTE:
...
- Front: condições que desabilitam o botão do robô na aba Planejar
...
- Entitlements/módulos: exigência de módulo além de `vendas` (ex.: "requer módulo Bot IA") no
  caminho do robô — remover do caminho do robô.
...
3. Front: botão do robô NUNCA fica desabilitado mudo — desabilitado sempre acompanha o motivo
