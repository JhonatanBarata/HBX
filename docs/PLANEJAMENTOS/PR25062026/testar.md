...
## Radar acha mais contato sozinho (novo — 25/06)
1. Entre em **Leads** → faça uma busca (Estado + Cidade + Segmento, ex.: PR / Curitiba / salão de beleza) → espere varrer.
2. Abra alguns leads que vieram **sem site** → agora muitos têm que mostrar **Instagram ou Facebook** encontrado (antes vinham vazios).
3. Não precisa configurar nada — é automático. (Quanto mais buscas, mais a base de contatos enche.)
...
## Card de Vendas nasce com a inteligência (novo — 25/06)
1. Entre em **Leads** → puxe um lead pra sua carteira.
2. Vá em **Vendas** → abra esse card.
3. Tem que mostrar **Instagram/Facebook, a nota (score) e o canal recomendado** — não pode vir pelado. (Se o lead tinha Instagram no Radar, aparece aqui também.)

## Excluir card pergunta o motivo (novo — 25/06)
1. Entre em **Vendas** → selecione um ou mais cards → clique em **Excluir**.
2. Tem que abrir uma **janelinha perguntando o motivo**: "Só excluir" ou "Resultado não satisfatório".
3. Escolha **"Só excluir"** → o card sai da sua lista, mas **volta a aparecer** quando você buscar de novo na mesma cidade/segmento (ele volta pra prateleira).
4. Em outro card escolha **"Resultado não satisfatório"** → esse **não volta** pra você (mas continua disponível pra outras empresas).

## Liberar "Excluir card" pro vendedor (novo — 25/06)
1. Entre no **Gerencial** → aba **Equipe** → na linha de um vendedor clique em **"Acessos"**.
2. Abre um painel com **todas as permissões agrupadas por módulo**, com busca.
3. Procure **"Excluir card"** → ligue → **Salvar acessos**.
4. Agora esse vendedor consegue excluir card em Vendas (antes dava erro de bloqueio).

## Fechar venda agora exige cadastro (novo — 25/06)
1. Entre em **Vendas** ou **Atendimento** → abra um card → **"Fechar venda"**.
2. Antes de **"Gerar link"**, é obrigatório ter **nome e telefone** do cliente — se faltar, o botão fica **travado** com um aviso "obrigatório".
3. Preencha → o botão libera → gera o link normal. (A comissão continua igual.)
