...
function buildRemoteReleaseScript(config, services) {
  ...
  const lines = [
    ...
    '}',
    'deploy_frontend_pm2() {',
    '  echo "Deploy frontend via PM2..."',
    '  if [ ! -d "$APP_DIR/frontend" ]; then echo "Pasta frontend nao encontrada em $APP_DIR/frontend; pulando."; return 0; fi',
    '  cd "$APP_DIR/frontend"',
    ...
    '      npm i -g pm2',
    '    fi',
    '    pm2 restart hbx-frontend 2>/dev/null || pm2 start npm --name hbx-frontend -- run start',
    '    pm2 save',
    '  else',
    '    echo "package.json nao encontrado em frontend, pulando deploy";',
    '  fi',
    '}',
