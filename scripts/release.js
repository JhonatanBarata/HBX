...
function buildRemoteReleaseScript(config, services) {
  ...
  const lines = [
    ...
    'remove_root_env HBX_FACTORY_MIN_ENGINES',
    'remove_root_env HBX_FACTORY_MAX_ENGINES',
    'remove_root_env HBX_FACTORY_MEMORY_SOFT_PRESSURE_PERCENT',
    'remove_root_env HBX_FACTORY_MEMORY_HARD_PRESSURE_PERCENT',
    'remove_root_env HBX_FACTORY_MEMORY_PANIC_PRESSURE_PERCENT',
    ...
    'export HBX_LIST_ENGINE_COUNT="$(awk -F= \'/^HBX_LIST_ENGINE_COUNT=/{print substr($0, length("HBX_LIST_ENGINE_COUNT")+2); exit}\' .env)"',
    'if [ -z "$HBX_LIST_ENGINE_COUNT" ]; then export HBX_LIST_ENGINE_COUNT="$HBX_ENGINE_COUNT"; fi',
    'case "$HBX_LIST_ENGINE_COUNT" in *[!0-9]*|"") echo "Aviso: HBX_LIST_ENGINE_COUNT invalido no .env; usando $HBX_ENGINE_COUNT."; export HBX_LIST_ENGINE_COUNT="$HBX_ENGINE_COUNT";; esac',
    'if [ "$HBX_LIST_ENGINE_COUNT" -lt 1 ]; then export HBX_LIST_ENGINE_COUNT=1; fi',
    'if [ "$HBX_LIST_ENGINE_COUNT" -gt "$HBX_ENGINE_COUNT" ]; then export HBX_LIST_ENGINE_COUNT="$HBX_ENGINE_COUNT"; fi',
    ...
    'upsert_root_env HBX_ENGINE_URLS "$(hbx_engine_urls)"',
    ...
    '    -e HBX_LIST_ENGINE_COUNT="${HBX_LIST_ENGINE_COUNT:-$HBX_ENGINE_COUNT}" \\',
    ...
    '  if [ "$(docker exec hbx-backend printenv HBX_LIST_ENGINE_COUNT)" != "${HBX_LIST_ENGINE_COUNT:-$HBX_ENGINE_COUNT}" ]; then echo "ERRO: HBX_LIST_ENGINE_COUNT nao reflete a frota no hbx-backend (paralelismo cairia para 1 motor)."; exit 1; fi',
