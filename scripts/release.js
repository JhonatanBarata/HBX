...
function buildRemoteReleaseScript(config, services) {
  ...
  const lines = [
    ...
    '  if [ -n "$BACKEND_URL" ]; then require_http_ok "$BACKEND_URL/health" "API publica /health pre-release"; webhook_code="$(http_status "$BACKEND_URL/webhooks/webwhats/events")"; if [ "$webhook_code" = "502" ] || [ "$webhook_code" = "000" ]; then echo "ERRO: webhook Webwhats publico indisponivel HTTP $webhook_code"; exit 1; fi; echo "Webhook Webwhats publico nao esta 502: HTTP $webhook_code"; fi',
    '  validate_webwhats_runtime',
    '}',
    'final_healthchecks() {',
    ...
    '    echo "Aguardando frontend Docker ($i/30)..."',
    '    sleep 2',
    '  done',
