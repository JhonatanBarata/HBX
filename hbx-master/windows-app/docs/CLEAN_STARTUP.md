# HBX Owner — limpeza de inicialização
...
2. **Atalhos duplicados no Startup**: se existem dois atalhos chamando `hbx_owner_app.py`, o Windows abre duas janelas do app.
3. **Uso de `python.exe` direto**: chamar o app por `python hbx_owner_app.py` abre console. Se há atalho, CMD e PowerShell também chamando, aparecem múltiplas janelas.
...
- bloqueia segunda instância do HBX Owner;
...
- remove atalhos antigos do HBX Owner no Startup;
- cria um único atalho `HBX Owner.lnk` usando `pythonw.exe`, sem console.
...
Para encerrar duplicados detectados do próprio HBX Owner:
...
Não deixe Startup, CMD, PowerShell ou atalho antigo chamando `hbx_owner_app.py` diretamente.
