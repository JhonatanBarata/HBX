...
const PORT = Number(process.env.HBX_MASTER_LOCAL_AGENT_PORT || 3107);
const TOKEN = String(process.env.HBX_MASTER_LOCAL_TOKEN || "").trim();
...
if (!TOKEN) {
  console.error("HBX_MASTER_LOCAL_TOKEN nao configurado. Configure a variavel de ambiente antes de iniciar.");
  process.exit(1);
}
...
server.listen(PORT, HOST, () => {
  console.log(`HBX Master Local Agent em http://${HOST}:${PORT}`);
});
