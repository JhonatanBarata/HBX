...
test.describe("login e cadastro mobile", () => {
  ...
  test("cadastro mobile mostra dois trials e plano empresarial como notebook", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name !== "mobile-chromium", "Fluxo validado no viewport mobile.");
    ...
    await expect(plans.getByText("R$ 0,00")).toHaveCount(2);
    await expect(plans.getByText(/Após 1º mês/)).toHaveCount(2);
